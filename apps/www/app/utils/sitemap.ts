...
const buildNodes = (routes: Routes) => {
  ...
  for (const node of routes) {
    ...
    if (node.lastmod)
      nodes.push(
        `<lastmod>${node.lastmod.format('YYYY-MM-DDThh:mm:ssZ')}</lastmod>`
      );
...
const getLoc = (path: string) =>
  `${process.env.VERCEL_ENV !== 'production' ? 'https://rippingyard-com.vercel.app/' : 'https://www.rippingyard.com/'}${path}`;
