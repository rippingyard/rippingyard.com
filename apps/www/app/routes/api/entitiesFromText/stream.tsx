...
import { tempContentStorage } from './storage';
...
export const loader = async ({ request }: { request: Request }) => {
  ...
  if (!contentId || !tempContentStorage[contentId]) {
    return new Response(
      JSON.stringify({ error: 'Invalid or expired content ID' }),
      {
        status: 400,
        headers: {
  ...
  // 保存されたcontentを取得
  const { content } = tempContentStorage[contentId];
